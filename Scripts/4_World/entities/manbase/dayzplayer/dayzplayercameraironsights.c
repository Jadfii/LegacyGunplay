...
{
	protected float m_breathFovModifier = 0.85;
	...
	override float HoldBreathFOVEffect(float pDt)
	{
		...
		{
			m_fFovAbsolute = Math.SmoothCD(m_fFovAbsolute, GameConstants.DZPLAYER_CAMERA_FOV_IRONSIGHTS * m_breathFovModifier, m_fFovAbsVel, 0.3, 1000, pDt);
		}
...
modded class DayZPlayerCameraOptics extends DayZPlayerCameraIronsights
{
	...
	override float HoldBreathFOVEffect(float pDt)
	{
		...
		if (optics)
		{
			...
			{
				m_fFovAbsolute = Math.SmoothCD(m_fFovAbsolute, GameConstants.DZPLAYER_CAMERA_FOV_EYEZOOM, m_fFovAbsVel, 0.3, 1000, pDt);
			}
			else if (m_isEntering) //sets FOV immediatelly to avoid "floating camera" effect
			{
